...
##class findClosestIntersection(object):
##    def __init__(self):
##        self.intersect = None
##        self.closestIntersect = None
##
##    def __call__(self, segments):
##        self.intersect = getIntersection(ray, segment)
##        if self.closestIntersect is None or self.intersect[2]<self.closestIntersect[2]:
##                self.closestIntersect = list(self.intersect)
##        return self.intersect
        

def findClosestIntersection(ray, segments):
...
##    canvas.fill([238,238,238])
    ...
    *uniquePoints,=map(list,{*map(tuple,points)})
    ...
    for j in range(len(uniqueAngles)):
        ...
        closestIntersect = None
        pool = Pool(5)
##        closestIntersect = pool.map(findClosestIntersection, (ray, segments))
        intersects = pool.map(getIntersection, (ray, segments))
        sorted_intersects = sorted(intersects, key=lambda x: x[1])
##        if intersect is None:
##            continue
        if closestIntersect is None or intersect[2]<closestIntersect[2]:
            closestIntersect = list(intersect)
...
##        closestIntersect = findClosestIntersection(ray, segments)
